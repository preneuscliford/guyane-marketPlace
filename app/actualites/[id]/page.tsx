...
type Comment = Database["public"]["Tables"]["comments"]["Row"] & {
  profiles: Database["public"]["Tables"]["profiles"]["Row"];
  likes?: { user_id: string }[];
  comments?: number; // nombre de sous-commentaires
};
        ...
        .select(
          `
              *,
              ...
              likes (user_id),
              likes: (select cast(count(*) as int) from likes where comment_id = comments.id),
              comments: (select cast(count(*) as int) from comments where parent_comment_id = comments.id)
            `
        )
