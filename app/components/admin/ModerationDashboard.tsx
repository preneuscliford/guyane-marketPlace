...
export default function ModerationDashboard() {
          ...
          (data || []).map(async (report) => {
            try {
              ...
              return {
                ...
                reported_user: reportedUser || { username: 'Utilisateur inconnu' }
              };
            } catch (err) {
              console.warn('Erreur lors de l\'enrichissement du signalement:', err);
              return {
                ...report,
   ...
   */
  const applyModerationAction = async (report: Report, action: ModerationAction) => {
    const { reported_content_type, reported_content_id, reported_user_id } = report;

    switch (action.action_type) {
      case 'hide':
        // Masquer le contenu
        if (['post', 'comment', 'announcement', 'service'].includes(reported_content_type)) {
          const tableName = reported_content_type === 'post' ? 'posts' : 
                           reported_content_type === 'comment' ? 'comments' :
                           reported_content_type === 'announcement' ? 'announcements' : 'services';
        ...
        await supabase
          ...
          .insert({
            ...
            banned_until: action.duration_hours ? 
              new Date(Date.now() + action.duration_hours * 60 * 60 * 1000).toISOString() : 
              null
          });
        break;
  ...
  const getContentIcon = (contentType: string) => {
    const IconComponent = CONTENT_TYPE_ICONS[contentType as keyof typeof CONTENT_TYPE_ICONS] || FileText;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gérez les signalements et modérez le contenu</p>
        </div>
      ...
      <div className="flex items-center justify-between bg-gray-50 px-4 py-2 rounded-lg">
        ...
        {(filterStatus !== 'all' || filterContentType !== 'all' || filterDateRange !== 'all') && (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 capitalize">{report.reported_content_type}</span>
                    </div>
                    <div>
                      <span className="font-medium">Utilisateur signalé:</span>
                      <span className="ml-2">{report.reported_user.username}</span>
                    </div>
                  </div>
                  ...
                  {report.moderator_notes && (
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <span className="font-medium text-sm">Notes du modérateur:</span>
                      <p className="mt-1 text-gray-600">{report.moderator_notes}</p>
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            ...
            {selectedReport && (
              <div className="bg-gray-50 p-3 rounded-lg border">
                <div className="text-sm text-gray-700">
                  <span className="font-medium">Signalement:</span> {REPORT_REASONS[selectedReport.reason as keyof typeof REPORT_REASONS] || selectedReport.reason}
                </div>
                <div className="text-sm text-gray-600 mt-1">
                  <span className="font-medium">Utilisateur signalé:</span> {selectedReport.reported_user?.username || 'Utilisateur inconnu'}
                </div>
              </div>
            )}
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-3">
              ...
              <Select
                ...
                onValueChange={(value) => setActionForm(prev => ({ ...prev, action_type: value as any }))}
              >
                ...
                <SelectContent>
                  <SelectItem value="hide">
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">Le contenu sera caché mais pas supprimé</div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem value="delete">
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <div>
                        <div className="font-medium">Supprimer le contenu</div>
                        <div className="text-xs text-gray-500">Le contenu sera définitivement supprimé</div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem value="ban_user">
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <div>
                        <div className="font-medium">Bannir l'utilisateur</div>
                        <div className="text-xs text-gray-500">L'utilisateur ne pourra plus accéder à la plateforme</div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem value="warn_user">
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <div>
                        <div className="font-medium">Avertir l'utilisateur</div>
                        <div className="text-xs text-gray-500">Envoyer un avertissement sans autre action</div>
                      </div>
                    </div>
                  </SelectItem>
                  <SelectItem value="restore">
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <div>
                        <div className="font-medium">Restaurer le contenu</div>
                        <div className="text-xs text-gray-500">Rendre le contenu visible à nouveau</div>
                      </div>
                    </div>
                  </SelectItem>
                </SelectContent>
            ...
            {actionForm.action_type === 'ban_user' && (
              <div className="space-y-3 p-4 bg-red-50 border border-red-200 rounded-lg">
                <Label className="text-sm font-medium text-red-800">Durée du bannissement (heures)</Label>
                <Input
                  type="number"
                ...
                <p className="text-xs text-red-600">
                  ⚠️ Un bannissement permanent ne peut pas être annulé automatiquement
                </p>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              <Input
                ...
                required
                className={`${!actionForm.reason ? 'border-red-300' : 'border-gray-300'}`}
              />
              {!actionForm.reason && (
          ...
          <DialogFooter className="flex gap-3 pt-6 border-t">
            ...
            <Button
              ...
              className={`flex-1 sm:flex-none ${
                actionForm.action_type === 'ban_user' || actionForm.action_type === 'delete'
